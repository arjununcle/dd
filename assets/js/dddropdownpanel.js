...
function ddpanel(setting) {
  ...
  if (
    setting.stateconfig.persiststate &&
    ddpanel.getCookie(setting.ids[0]) == "down"
  ) {
    setting.dir = "down";
  }
  if (setting.dir == "up") {
    //if "up", output CSS to hide panel contents
    document.write('<style type="text/css">\n');
    document.write(
      "#" +
        setting.ids[1] +
        "{height:" +
        parseInt(setting.stateconfig.initial) +
        "px; overflow:hidden}\n"
    );
  ...
  var thispanel = this;
  if (
    window.addEventListener //if non IE browsers, initialize panel window.onload
  )
    ddpanel.addEvent(
      window,
      function(e) {
        thispanel.initpanel(setting);
      },
      "load"
    ); //else if IE, add 100 millisec after window.onload before initializing
  else
    ddpanel.addEvent(
      window,
      function(e) {
...
ddpanel.addEvent = function(target, functionref, tasktype) {
  ...
  var i =
    this.events_array.push({
      //store event info in ddpanel.events_array[] and return current event's index within array
      target: target,
  ...
  target[evtaction](
    this.events_array[i].tasktype,
    this.events_array[i].listener,
...
ddpanel.prototype = {
  togglepanel: function(dir) {
    //public function that toggles the panel's state. Optional dir parameter ("up" or "down") to explicitly set state.
    var setting = this.setting;
    setting.dir = dir || (setting.dir == "up" ? "down" : "up");
    var pcontent = setting.pcontent,
      dir = setting.dir;
    pcontent._currentheight =
      dir == "down" ? pcontent._actualheight : setting.stateconfig.initial;
    pcontent.style.height = pcontent._currentheight + "px";
    if (setting.pointerimage.enabled) {
      setting.arrow.src =
        setting.pointerimage.src[setting.dir == "down" ? 1 : 0];
  ...
  togglepanelplus: function(dir) {
    //public function that toggles the panel's state w/ animation. Optional dir parameter ("up" or "down") to explicitly set state.
    var setting = this.setting;
    setting.dir = dir || (setting.dir == "up" ? "down" : "up");
    if (setting.pointerimage.enabled) setting.arrow.style.visibility = "hidden";
  ...
  revealcontent: function() {
    var setting = this.setting;
    var pcontent = setting.pcontent,
      curH = pcontent._currentheight,
      maxH = pcontent._actualheight,
      minH = setting.stateconfig.initial,
      steps = setting.animate.steps,
      dir = setting.dir;
    if ((dir == "down" && curH < maxH) || (dir == "up" && curH > minH)) {
      var newH =
        curH + (Math.round((maxH - curH) / steps) + 1) * (dir == "up" ? -1 : 1);
    ...
    var thispanel = this;
    setting.revealtimer = setTimeout(function() {
      thispanel.revealcontent();
    }, 10);
  },

  initpanel: function() {
    var setting = this.setting;
    var pcontainer = (setting.pcontainer = document.getElementById(
      setting.ids[0]
    ));
    ...
    pcontent._currentheight =
      setting.dir == "down"
        ? pcontent._actualheight
        : setting.stateconfig.initial;
    var thispanel = this;
    ddpanel.addEvent(
      tdiv,
      function(e) {
        //assign click behavior when toggle DIV tab is clicked on
        if (setting.animate.enabled) thispanel.togglepanelplus();
        else thispanel.togglepanel();
    ...
    if (setting.pointerimage.enabled) {
      var pointer1 = new Image(),
        pointer2 = new Image();
      pointer1.src = setting.pointerimage.src[0];
      pointer2.src = setting.pointerimage.src[1];
      setting.arrow = ddpanel.addpointer(
        tdiv.getElementsByTagName("span")[0],
        "pointerimage",
        setting.pointerimage.src[setting.dir == "down" ? 1 : 0]
      );
    }
    if (setting.closepanelonclick.enabled) {
      //assign click behavior when panel content is clicked on (links within panel or elements with class "closepanel"
      ddpanel.addEvent(
        pcontent,
        function(e) {
          var target = e.srcElement || e.target;
          if (
            /(^|\s+)closepanel($|\s+)/.test(target.className) ||
            target.tagName == "A" ||
...
var defaultpanel = new ddpanel({
  ...
  pointerimage: {
    enabled: true,
    src: ["assets/images/arrow-down.gif", "assets/images/arrow-up.gif"]
  },
